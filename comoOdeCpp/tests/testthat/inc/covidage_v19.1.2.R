...
# file_path <- paste0(getwd(),"/Template_CoMoCOVID-19App_v18.xlsx")  
...
multi_runs_pp_only <- function(out0, times, parameters, input, iterations = 1){
  ...
  results$mean_cases<-matrix(0,nrow = length(times),ncol = 37*A+1)
  results$min_cases<-matrix(0,nrow = length(times),ncol = 37*A+1)
  results$max_cases<-matrix(0,nrow = length(times),ncol = 37*A+1)
